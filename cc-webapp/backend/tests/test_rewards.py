...
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_rewards.db"
...
USER_ID_FOR_REWARDS_TESTS = 201

def test_get_rewards_first_page(client, db_session: Session):
...
def test_get_rewards_second_page(client, db_session: Session):
    ...
    assert data["total_rewards"] == 25
    assert data["total_pages"] == 3
    assert data["rewards"][0]["reward_id"] == seeded_rewards[10].id

def test_get_rewards_last_page_partial(client, db_session: Session):
...
def test_get_rewards_page_out_of_bounds(client, db_session: Session):
...
def test_get_rewards_no_rewards(client, db_session: Session):
...
def test_get_rewards_user_not_found(client, db_session: Session):
    response = client.get("/api/users/9999/rewards")
    assert response.status_code == 404, response.text
    assert "존재하지 않는 사용자" in response.json()["detail"]

def test_get_rewards_default_pagination(client, db_session: Session):

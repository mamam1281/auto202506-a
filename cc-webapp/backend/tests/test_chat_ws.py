...
def test_chat_websocket_success(client, patch_service):
...
def test_chat_websocket_invalid_token(client):
    with pytest.raises(Exception):
        with client.websocket_connect("/ws/chat", headers={"Authorization": "Bearer bad"}):
            pass

def test_chat_websocket_bad_message(client, patch_service):
